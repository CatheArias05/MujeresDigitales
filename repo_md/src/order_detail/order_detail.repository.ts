...
export class OrderDetailRepository {
  ...
  async create(detail: Partial<OrderDetail> | CreateOrderDetailDto): Promise<OrderDetail> {
    ...
    if (typeof d.subtotal !== 'undefined' && d.subtotal !== total) {
      throw new Error('Los subtotales no coinciden, hay un error en el calculo');
    }
  ...
  async update(id: string, data: Partial<OrderDetail> | UpdateOrderDetailDto): Promise<OrderDetail> {
    const payload: Partial<OrderDetail> = { ...data };
    const d: any = data as any;
    if (typeof d.uuid_product === 'string' || (d.product && d.product.uuid)) {
      const productUuid = typeof d.uuid_product === 'string' ? d.uuid_product : d.product.uuid;
      const product = await this.productDataBase.findOne({ where: { uuid: productUuid } });
      if (!product) {
      ...
      const total = totalProductsPrice + totalIva - totalDiscount;
      if (typeof d.subtotal !== 'undefined' && d.subtotal !== total) {
        throw new Error('Los subtotales no coinciden, hay un error en el cálculo');
      }
      (payload as any).product = product;
      (payload as any).subtotal = total;
      delete (payload as any).uuid_product;
    ...
    if (typeof d.uuid_order === 'string' || (d.order && d.order.uuid_order)) {
      const orderUuid = typeof d.uuid_order === 'string' ? d.uuid_order : d.order.uuid_order;
      const order = await this.orderDataBase.findOne({ where: { uuid_order: orderUuid } });
      if (!order) {
