...
describe('BusinessService', () => {
  ...
  beforeEach(async () => {
    const module: TestingModule = await Test.createTestingModule({
      providers: [BusinessService, { provide: BusinessRepository, useValue: repo }],
    }).compile();
  ...
  it('getBusinessByIdService lanza NotFound si no existe', async () => {
    ...
    await expect(service.getBusinessByIdService('x')).rejects.toBeInstanceOf(NotFoundException);
  });

  it('postCreateBusinessService valida requeridos y lanza BadRequest', async () => {
    await expect(service.postCreateBusinessService({} as any)).rejects.toBeInstanceOf(BadRequestException);
  });

  it('deleteBusinessService lanza Conflict si ya inactivo', async () => {
    (repo.getBusinessByIdRepository as any).mockResolvedValue({ estado: false });
    await expect(service.deleteBusinessService('x')).rejects.toBeInstanceOf(ConflictException);
  });
});