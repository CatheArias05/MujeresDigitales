...
describe('ProductService', () => {
  ...
  it('getProductById lanza NotFound si no existe', async () => {
    ...
    await expect(service.getProductById('x')).rejects.toBeInstanceOf(NotFoundException);
  });

  it('postCreateProduct lanza Conflict si code ya existe', async () => {
    (productRepo.findProductByCode as any).mockResolvedValue({ uuid: '1' });
    await expect(service.postCreateProduct({ productCode: 1, name: 'p', categoryUuid: 'c', businessUuid: 'b', basePrice: 1 } as any)).rejects.toBeInstanceOf(ConflictException);
  });
  ...
  it('postCreateProduct crea cuando todo válido', async () => {
    ...
    (categoryRepo.findCategoryByUuid as any).mockResolvedValue({ uuid: 'c', isActive: true });
    (businessRepo.findBusinessByUuid as any).mockResolvedValue({ uuid: 'b', estado: true });
    (productRepo.createProduct as any).mockResolvedValue({ message: 'ok' });
    const res = await service.postCreateProduct({ productCode: 1, name: 'p', categoryUuid: 'c', businessUuid: 'b', basePrice: 1 } as any);
