...
describe('ShippingService', () => {
  ...
  it('putUpdateShippingService lanza NotFound si no existe', async () => {
    (repo.getShippingByIdRepository as any).mockResolvedValue(null);
    await expect(service.putUpdateShippingService('x', {} as any)).rejects.toBeInstanceOf(NotFoundException);
  });

  it('putUpdateShippingService delega al repositorio', async () => {
    (repo.getShippingByIdRepository as any).mockResolvedValue({ uuid: 's', estado_envio: EstadoEnvio.PENDIENTE });
    (repo.putUpdateShippingRepository as any).mockResolvedValue({ message: 'ok' });
    const res = await service.putUpdateShippingService('s', { estado_envio: EstadoEnvio.EN_CAMINO } as any);
  ...
  it('getShippingByOrderUuidService lanza NotFound si no existe', async () => {
    ...
    await expect(service.getShippingByOrderUuidService('o')).rejects.toBeInstanceOf(NotFoundException);
  });
