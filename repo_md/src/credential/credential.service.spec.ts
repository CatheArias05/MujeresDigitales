...
describe('CredentialService', () => {
  ...
  beforeEach(async () => {
    const module: TestingModule = await Test.createTestingModule({
      providers: [CredentialService, { provide: CredentialRepository, useValue: repo }],
    }).compile();
  ...
  it('updateCredentialService lanza NotFound si no existe', async () => {
    ...
    await expect(service.updateCredentialService({ uuid: 'x' } as any)).rejects.toBeInstanceOf(NotFoundException);
  });
  ...
  it('updateCredentialService lanza Conflict si user_name ya existe', async () => {
    ...
    await expect(service.updateCredentialService({ uuid: '1', user_name: 'u' } as any)).rejects.toBeInstanceOf(
      ConflictException,
    );
  });
  ...
  it('updateCredentialService actualiza si válido', async () => {
    ...
    (repo.updateCredentialRepository as any).mockResolvedValue({ uuid: '1', user_name: 'ok' });
    const res = await service.updateCredentialService({ uuid: '1', user_name: 'ok' } as any);
