...
describe('OrderService', () => {
  ...
  it('createOrderService delega al repositorio', async () => {
    ...
    const res = await service.createOrderService({ user_id: 'u', total: 1, shipping_price: 0, pay_method: 'cash' } as any);
    expect(res.uuid_order).toBe('1');
  });

  it('update lanza error cuando no existe', async () => {
    (repo.getById as any).mockResolvedValue(null);
    await expect(service.update('x', {} as any)).rejects.toThrow('Orden no encontrada');
  });
